...
export default function PhotosUploader({ addedPhotos, onChange }) {
  ...
  function uploadPhoto(ev) {
    const MAX_PHOTOS = 5; // Maximum number of photos
    const files = ev.target.files;
    const data = new FormData();
  
    // Checking if there are already 5 photos
    if (files.length + addedPhotos.length > MAX_PHOTOS) {
      toast.error(`You can only upload ${MAX_PHOTOS} photos`);
      return;
    }
    ...
    axios
      .post(`${import.meta.env.VITE_BASE_URI}/upload`, data, {
  ...
  async function removePhoto(ev,filename) {
    ...
    await axios.post(`${import.meta.env.VITE_BASE_URI}/remove`,{
      data
    });
  ...
  return (
    <>
      <div className="mt-2 grid gap-2 grid-cols-3 md:grid-cols-4 lg:grid-cols-6">
        ...
        <label className={`h-32 cursor-pointer flex items-center gap-1 justify-center border bg-transparent rounded-2xl p-2 text-lg text-gray-600 ${isUploading ? "animate-pulse" : ""}`}>
